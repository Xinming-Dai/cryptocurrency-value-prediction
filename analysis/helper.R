...
# some large corporations which make significant impact on cryptocurrency and obtain their tickers
...
# make a column calculate the percentage change from the other day
pc_col <- function(x){
...
create_data <- function(start_date, end_date, ticks = tickers, ts = T, lags = 3, difference_cryptos = T){
  ...
  # date and their adjusted return value
  prices <- tq_get(ticks,
                   from = start_date,
                   to = end_date,
                   get = "stock.prices")
  ...
  # are all the tickers we select at the beginning with their adjusted return 
  for ( i in ticks){
  ...
  # finally, we convert all of prices to return by calling pc_col() function which build previosuly
  returns <- priceH
  for(i in ticks){
...
# function for training, testing, and predicting for randomforest model, similar sturcutee as above
